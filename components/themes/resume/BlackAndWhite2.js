...
export default function BlackAndWhite({
  ...
  useEffect(() => {
    ...
    return () => inputs.forEach(input => input.removeEventListener('input', () => adjustInputWidth(input)));
  }, [resumeData]);
...
const renderExpandingExperienceInput = (value, onChange, placeholder = "", style) => (
    <input
      style={{
        ...styles.expandingInput,
        ...style,
  ...
  const renderSection = (title, data, renderItem, addItem, sectionKey) => (
    ...
    >
      <h1 style={{...styles.sectionTitle, color: fullNameColor, fontSize: '1rem', marginBottom: '1px'}}>{title}</h1>
  ...
  return (
    <div ref={resumeRef} style={{ padding: '1px' }}>
      <main style={{...styles.main, ...(generatingSection ? styles.generatingSection : {})}}>
        ...
        <div style={styles.socialInfo}>
          <div style={styles.socialInfoItem}>
            ...
            <input
              ...
              value={resumeData.linkedin ? (resumeData.linkedin.includes('linkedin.com') ? '/in/' + resumeData.linkedin.split('/in/')[1] : resumeData.linkedin) : ""}
              onChange={(e) => {
                const value = e.target.value;
                const newValue = value.startsWith('/in/') ? 'https://www.linkedin.com' + value : value;
                handleInputChange({ target: { name: 'linkedin', value: newValue } }, setResumeData);
              }}
              onClick={(e) => {
          ...
          <div style={styles.socialInfoItem}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" style={{width: '0.75rem', height: '0.75rem', flexShrink: 0}} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" style={{ fill: fullNameColor }} />
              <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" style={{ fill: fullNameColor }} />
            </svg>
            <input
              style={styles.autoExpand}
              className="auto-expand"
        ...
        <section 
          className={`relative group ${generatingSection === 'summary' ? 'generating' : ''}`}
        ...
        >
          <h1 style={{...styles.sectionTitle, color: fullNameColor}}>{t.summary}</h1>
          <textarea
            ref={summaryRef}
        ...
        {renderSection(
          ...
          (experience, index) => (
            <li
              key={index}
              onMouseEnter={() => setHoveredStates(prev => ({...prev, experience: index}))}
              onMouseLeave={() => setHoveredStates(prev => ({...prev, experience: null}))}
              style={{ 
            ...
            >
              <div style={{...styles.experienceItemContent, padding: '5px 0'}}>
                {renderExpandingExperienceInput(
                  experience.position,
                  (e) => handleExperienceChange(index, 'position', e.target.value, setResumeData, resumeData),
                  t.position,
                  { fontWeight: 'bold', color: fullNameColor }
                )}
                <p>|</p>
                {renderExpandingExperienceInput(
                  experience.company,
                  (e) => handleExperienceChange(index, 'company', e.target.value, setResumeData, resumeData),
                  t.company,
                  { fontWeight: 'bold' }
                )}
                <p>|</p>
                {renderExpandingExperienceInput(
                  experience.location,
                  (e) => handleExperienceChange(index, 'location', e.target.value, setResumeData, resumeData),
                  t.location,
                  { fontWeight: 'bold', color: fullNameColor}
                )}
                <p>|</p>
                {renderExpandingExperienceInput(
                  experience.duration,
                  (e) => handleExperienceChange(index, 'duration', e.target.value, setResumeData, resumeData),
                  t.duration,
                  { fontWeight: 'bold' }
                )}
              </div>
              <div style={{...styles.responsibilities, marginTop: '0', marginBottom: '0'}}>
                <textarea
                  style={{...styles.textarea, marginTop: '0', marginBottom: '0'}}
              ...
              {hoveredStates.experience === index && (
                <>
                  <button
                    ...
                    style={{...styles.removeButton, bottom: '5px', right: '5px'}}
                  >
                    <FaTrash style={{ width: '1rem', height: '1rem' }} />
                  </button>
                  <button
                    onClick={() => setRegenerateFields(prevState => ({ ...prevState, [index]: !prevState[index] }))}
                    style={{position: 'absolute', top: '-25px', right: '5px', background: 'white', border: '1px solid blue', color: 'blue', padding: '2px 10px', borderRadius: '5px'}}
                  >
                    {t.regenerate}
                  </button>
                  ...
                  <button
                    ...
                    style={{position: 'absolute', top: '-25px', left: '5px', background: 'white', border: '1px solid green', color: 'green', padding: '2px 10px', borderRadius: '5px'}}
                  >
                    {t.moreOptions}
                  </button>
        ...
        {renderSection(
          ...
          (edu, index) => (
            <li 
              key={index} 
              onMouseEnter={() => setHoveredStates(prev => ({...prev, education: index}))}
              onMouseLeave={() => setHoveredStates(prev => ({...prev, education: null}))}
              style={{ 
                position: 'relative', 
                marginBottom: '1px',
            ...
            >
              ...
              <input
                style={{ fontStyle: 'italic', marginLeft: '10px', width: '50%' }}
        ...
        {renderSection(
          ...
          (skill, index) => (
            <div 
              ...
              onMouseEnter={() => setHoveredStates(prev => ({...prev, skill: index}))}
              onMouseLeave={() => setHoveredStates(prev => ({...prev, skill: null}))}
            >
              {renderExpandingInput(
                skill,
                (e) => handleSkillChange(index, e.target.value, setResumeData, resumeData),
                t.skill
              )}
              {hoveredStates.skill === index && (
                <button
                  onClick={() => removeSkill(index, setResumeData, resumeData)}
                  style={{ ...styles.removeButton, position: 'static', marginLeft: '5px' }}
                >
        ...
        {sectionsVisibility.hobbies && renderSection(
          t.hobbies,
          resumeData.hobbies || [],
          (hobby, index) => (
            <div 
              ...
              onMouseEnter={() => setHoveredStates(prev => ({...prev, hobby: index}))}
              onMouseLeave={() => setHoveredStates(prev => ({...prev, hobby: null}))}
            >
              {renderExpandingInput(
                hobby || "",
                (e) => handleHobbyChange(index, e.target.value, setResumeData, resumeData),
                t.hobby
              )}
              {hoveredStates.hobby === index && (
                <button
        ...
        {sectionsVisibility.software && renderSection(
          t.software,
          resumeData.software || [],
          (software, index) => (
            <div 
              ...
              onMouseEnter={() => setHoveredStates(prev => ({...prev, software: index}))}
              onMouseLeave={() => setHoveredStates(prev => ({...prev, software: null}))}
            >
              {renderExpandingInput(
                software || "",
                (e) => handleSoftwareChange(index, e.target.value, setResumeData, resumeData),
                t.software
              )}
              {hoveredStates.software === index && (
                <button
        ...
        {sectionsVisibility.languages && renderSection(
          t.languages,
          ...
          (language, index) => (
            <div 
              ...
              onMouseEnter={() => setHoveredStates(prev => ({...prev, language: index}))}
              onMouseLeave={() => setHoveredStates(prev => ({...prev, language: null}))}
            >
              {renderExpandingInput(
                language || "",
                (e) => handleLanguageChange(index, e.target.value, setResumeData, resumeData),
                t.language
              )}
              {hoveredStates.language === index && (
                <button
        ...
        {sectionsVisibility.certificates && renderSection(
          t.certificates,
          resumeData.certificates || [],
          (certificate, index) => (
            <li 
              key={index} 
              onMouseEnter={() => setHoveredStates(prev => ({...prev, certificate: index}))}
              onMouseLeave={() => setHoveredStates(prev => ({...prev, certificate: null}))}
              style={{ 
                position: 'relative', 
                marginBottom: '1px',
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1px' }}>
                {renderExpandingInput(
                  certificate.name || "",
                  (e) => handleCertificateChange(index, 'name', e.target.value, setResumeData, resumeData),
                  t.certificate
                )}
                <input
                  style={{ color: fullNameColor, marginLeft: '20px' }}
        ...
        {sectionsVisibility.extraSection && renderSection(
          t.extraSections,
          ...
          (extra, index) => (
            <li 
              ...
              onMouseEnter={() => setHoveredStates(prev => ({...prev, extra: index}))}
              onMouseLeave={() => setHoveredStates(prev => ({...prev, extra: null}))}
            >
              <input
                style={{ width: '100%', border: 'none', borderBottom: '1px solid #ccc', outline: 'none' }}
        ...
        {sectionsVisibility.extraDetailedSection && renderSection(
          ...
          (extraDetail, index) => (
            <li
              ...
              onMouseEnter={() => setHoveredStates(prev => ({...prev, extraDetailed: index}))}
              onMouseLeave={() => setHoveredStates(prev => ({...prev, extraDetailed: null}))}
            >
              <input
                style={{ 
                  width: '100%', 
              ...
              />
              <ul style={{ listStyleType: 'disc', paddingLeft: '20px', marginTop: '1px' }}>
                {(extraDetail.details || []).map((detail, detailIndex) => (
                  <li
                    key={detailIndex}
                    style={{ position: 'relative', marginBottom: '1px' }}
                    onMouseEnter={() => setHoveredStates(prev => ({...prev, extraDetailedDetail: {sectionIndex: index, detailIndex}}))}
                    onMouseLeave={() => setHoveredStates(prev => ({...prev, extraDetailedDetail: null}))}
                  >
                    <textarea
                      style={{ 
                        width: '100%',
                      ...
                      value={detail || ""}
                      onChange={(e) => handleExtraDetailedSectionDetailChange(index, detailIndex, e.target.value, setResumeData, resumeData)}
                      onInput={(e) => {
                        e.target.style.height = 'auto';
                        e.target.style.height = `${e.target.scrollHeight}px`;
                      }}
                    ...
                    {hoveredStates.extraDetailedDetail && 
                     hoveredStates.extraDetailedDetail.sectionIndex === index && 
                     hoveredStates.extraDetailedDetail.detailIndex === detailIndex && (
                      <button
                        onClick={() => removeExtraDetailedSectionDetail(index, detailIndex, setResumeData, resumeData)}
                        style={{ 
                          position: 'absolute', 
                          right: '-25px', 
                          top: '50%', 
